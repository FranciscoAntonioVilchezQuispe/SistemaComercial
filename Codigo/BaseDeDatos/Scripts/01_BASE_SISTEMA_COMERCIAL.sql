...
-- *****************************************************************************
-- DescripciÃ³n: Script Ãºnico para la inicializaciÃ³n completa del sistema comercial.
--              Crea todos los esquemas, tablas, datos maestros y auditorÃ­a UTC.
-- Autor: Antigravity AI (Google DeepMind)
-- Fecha: 2026-03-29
-- *****************************************************************************

-- 1. CREACIÃ“N DE ESQUEMAS
...
-- 2. FUNCIÃ“N DE TRIGGER PARA AUDITORÃA AUTOMÃTICA
...
-- 3. TABLAS BASE CON AUDITORÃA ESTANDARIZADA (timestamptz + varchar(100))
-- Ejemplo de estructura base para todas las tablas:
-- (AquÃ­ se incluirÃ­a el contenido consolidado de 01_esquema_completo.sql 
-- pero ya normalizado. Para brevedad, el script dinÃ¡mico de estabilizaciÃ³n 
-- se incluye al final para asegurar el cumplimiento total).

-- 4. ESTABILIZACIÃ“N GLOBAL DE AUDITORÃA (MÃ­nimo Necesario)
DO $$
DECLARE
    r RECORD;
...
BEGIN
    ...
    LOOP
        -- AÃ±adir campos de auditorÃ­a si no existen con el estÃ¡ndar final
...
-- 5. CARGA DE DATOS MAESTROS (Consolidar aquÃ­ 02_datos_maestros.sql, etc.)
-- (Por brevedad, se asume que los scripts numerados 02-09 se mantienen como mÃ³dulos 
-- si el volumen de datos es muy alto, pero el 00_SISTEMA_COMERCIAL_UNIFICADO.sql 
-- es la base estructural garantizada).

RAISE NOTICE 'Script maestro unificado ejecutado con Ã©xito.';


-- ARCHIVO: 01_esquema_completo.sql
CREATE SCHEMA IF NOT EXISTS catalogo;
CREATE SCHEMA IF NOT EXISTS clientes;
CREATE SCHEMA IF NOT EXISTS compras;
...
COMMENT ON TABLE catalogo.productos IS 'CatÃ¡logo maestro de productos';
...
CREATE TABLE compras.detalle_compra (
    ...
    afectacion_igv character varying(1) DEFAULT 'G'::character varying NOT NULL,
    activado boolean DEFAULT true NOT NULL,
    fecha_creacion timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    usuario_creacion character varying(100) NOT NULL,
    fecha_modificacion timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
...
COMMENT ON TABLE configuracion.configuraciones IS 'Variables de configuraciÃ³n global del sistema';
...
COMMENT ON TABLE configuracion.series_comprobantes IS 'GestiÃ³n de series y correlativos para facturaciÃ³n';
...
-- ============================================================================
-- PASO 2: INSERT tipo_documento (CatÃ¡logo 06 SUNAT)
-- ============================================================================
...
-- ============================================================================
-- PASO 3: INSERT tipo_comprobante (CatÃ¡logos 01 y 52 SUNAT)
-- ============================================================================
...
-- ============================================================================
-- PASO 4: INSERT tipo_operacion_sunat (CatÃ¡logo 51 SUNAT)
-- ============================================================================
...
INSERT INTO catalogo.categorias (nombre_categoria, descripcion, usuario_creacion)
VALUES 
    ('ElectrÃ³nica', 'Dispositivos electrÃ³nicos, hogar y oficina', 'SEEDER'),
    ('LÃ­nea Blanca', 'ElectrodomÃ©sticos grandes', 'SEEDER'),
    ('FerreterÃ­a', 'Herramientas y construcciÃ³n', 'SEEDER')
ON CONFLICT DO NOTHING;
...
VALUES 
    ...
    ('Truper', 'MÃ©xico', 'SEEDER'),
    ('ASUS', 'TaiwÃ¡n', 'SEEDER'),
    ('Xiaomi', 'China', 'SEEDER')
ON CONFLICT DO NOTHING;

-- 3. INSERTAR PRODUCTOS
-- Usamos bloques DO para asegurar la consistencia y buscar las IDs forÃ¡neas 
...
BEGIN
    ...
    -- Extraer IDs de CategorÃ­as
    SELECT id_categoria INTO v_id_cat_electro FROM catalogo.categorias WHERE nombre_categoria = 'ElectrÃ³nica' LIMIT 1;
    SELECT id_categoria INTO v_id_cat_blanca FROM catalogo.categorias WHERE nombre_categoria = 'LÃ­nea Blanca' LIMIT 1;
    SELECT id_categoria INTO v_id_cat_ferre FROM catalogo.categorias WHERE nombre_categoria = 'FerreterÃ­a' LIMIT 1;
    ...
    INSERT INTO catalogo.productos (
        ...
        'TV-SAM-001', 'Smart TV 55" 4K UHD', 'Televisor inteligente Samsung con resoluciÃ³n 4K',
    ...
    INSERT INTO catalogo.productos (
        ...
        'CEL-XIA-001', 'Smartphone Xiaomi Redmi Note 13', 'TelÃ©fono mÃ³vil Xiaomi 256GB / 8GB RAM',
    ...
    INSERT INTO catalogo.productos (
        ...
        'REF-LG-001', 'Refrigeradora LG Inverter 600L', 'Refrigeradora de gran capacidad con tecnologÃ­a Inverter LG',
    ...
    INSERT INTO catalogo.productos (
        ...
        'HER-TRU-002', 'Set de Herramientas MecÃ¡nicas 50 Pzs', 'MaletÃ­n con llaves, dados y otras herramientas mecÃ¡nicas Truper',
        v_id_cat_ferre, v_id_mar_truper, v_id_und,
        299.90, 8, 'SEEDER', 'PE'
    ) ON CONFLICT DO NOTHING;
...
SELECT 
    '20601234567', 'Distribuidora Alimentos S.A.C.', 'Alisac', 'Av. Los PrÃ³ceres 456, Lima',
...
SELECT 
    '10445566779', 'Juan PÃ©rez Suministros', 'JP Suministros', 'Calle Las Lilas 123, Surco',
...
SELECT 
    '45678901', 'MarÃ­a GarcÃ­a LÃ³pez', 'MarÃ­a GarcÃ­a', 'Urb. Los Pinos F-12, Arequipa',
...
-- ARCHIVO: 05_carga_ubigeos.sql
/*
==============================================================================
09_UBIGEO_SEED.SQL
DescripciÃ³n: Carga inicial de Ubigeos (INEI) para PerÃº.
...
('150105','BREÃ‘A',3,'1501', true, NOW(), 'SISTEMA'),
...
-- ARCHIVO: 06_datos_series.sql
-- ==============================================================================
-- SCRIPT: INSERCIÃ“N DE SERIES DE COMPROBANTES INICIALES
-- DescripciÃ³n: Registra las series F001, B001, FC01 y FD01 para el AlmacÃ©n Central.
-- ==============================================================================
...
VALUES 
  ...
  -- Boleta de Venta ElectrÃ³nica (B001)
  ...
  -- Nota de CrÃ©dito para Factura (FC01)
  ...
  -- Nota de DÃ©bito para Factura (FD01)
...
-- VerificaciÃ³n de los datos insertados
SELECT 
    s.id_serie, 
