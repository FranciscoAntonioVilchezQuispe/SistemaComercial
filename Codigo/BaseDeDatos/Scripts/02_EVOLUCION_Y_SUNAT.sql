...
-- SCRIPT DE CORRECIÃƒâ€œN PARA MÃƒâ€°TODOS DE PAGO (FASE 9)
-- Crea la tabla faltante en el esquema de configuraciÃƒÂ³n para el microservicio Configuracion.API
...
CREATE TABLE IF NOT EXISTS configuracion.metodos_pago (
    ...
    id_tipo_documento_pago BIGINT, -- RelaciÃƒÂ³n opcional con tipo de documento
...
('TARJETA', 'Tarjeta de DÃƒÂ©bito/CrÃƒÂ©dito', FALSE, 'SISTEMA'),
...
-- pero Configuracion.API usarÃƒÂ¡ exclusivamente esta nueva tabla centralizada.



-- ARCHIVO: 03_consolidar_metodos_pago.sql
-- SCRIPT DE CONSOLIDACIÃƒâ€œN DE MÃƒâ€°TODOS DE PAGO (FASE 10)
-- Unifica ventas.metodos_pago en configuracion.metodos_pago

DO $$ 
BEGIN
    -- 1. Asegurar que los cÃƒÂ³digos de ventas existan en configuraciÃƒÂ³n antes de re-mapear
    ...
    -- 3. Actualizar los IDs en ventas.pagos basados en el mapeo de cÃƒÂ³digos
    ...
    -- La cascada deberÃƒÂ­a encargarse de la secuencia si estaba ligada, pero por seguridad:
...
==============================================================================
Este script realiza:
1. CreaciÃƒÂ³n de CatÃƒÂ¡logos SUNAT (AfectaciÃƒÂ³n IGV, Motivos NC/ND).
2. AdaptaciÃƒÂ³n de tipo_documento para CatÃƒÂ¡logo 52 (Documento Relacionado).
3. CorrecciÃƒÂ³n de esquemas en ventas y compras (campos faltantes para FE).
4. CreaciÃƒÂ³n de vistas optimizadas para el frontend en el esquema 'vistas'.
==============================================================================
*/

-- ------------------------------------------------------------------------------
-- 1. CATÃƒÂLOGO 17: TIPO DE AFECTACIÃƒâ€œN AL IGV
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 2. ADAPTACIÃƒâ€œN TIPO_DOCUMENTO (CATÃƒÂLOGOS 06 Y 52)
-- ------------------------------------------------------------------------------
DO $$ 
BEGIN 
...
-- ------------------------------------------------------------------------------
-- 3. CATÃƒÂLOGOS 09 Y 10: MOTIVOS DE NC Y ND
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 4. CORRECCIÃƒâ€œN VENTAS.DETALLE_VENTA
-- ------------------------------------------------------------------------------
DO $$ 
BEGIN 
...
-- ------------------------------------------------------------------------------
-- 5. CORRECCIÃƒâ€œN COMPRAS.DETALLE_COMPRA
-- ------------------------------------------------------------------------------
...
SELECT 
    ...
    m.concepto, 
    m.monto, 
    'PEN' AS moneda, -- Por defecto segÃƒÂºn script
...
-- SCRIPT UNIFICADO: Insertar datos semilla + Consolidar MÃƒÂ©todos de Pago
...
('TARJETA', 'Tarjeta de DÃƒÂ©bito/CrÃƒÂ©dito', FALSE, TRUE, NOW(), 'SISTEMA'),
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'ventas' AND table_name = 'metodos_pago') THEN
        ...
        -- Actualizar IDs en ventas.pagos basados en mapeo de cÃƒÂ³digos
        UPDATE ventas.pagos p
        ...
        RAISE NOTICE 'Ã¢Å“â€¦ ConsolidaciÃƒÂ³n completada: ventas.metodos_pago eliminada, FK actualizada.';
    ELSE
        RAISE NOTICE 'Ã¢â€žÂ¹Ã¯Â¸Â ventas.metodos_pago no existe, solo se aplicÃƒÂ³ seed data.';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_ventas_pagos_metodo_config' AND table_schema = 'ventas') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'ventas' AND table_name = 'pagos' AND column_name = 'id_metodo_pago') THEN
            ...
            FOREIGN KEY (id_metodo_pago) 
            REFERENCES configuracion.metodos_pago(id_metodo_pago);
            RAISE NOTICE 'Ã¢Å“â€¦ FK fk_ventas_pagos_metodo_config creada.';
        END IF;
    END IF;
END $$;
...
/*
==============================================================================
SCRIPT: SINCRONIZACIÃƒâ€œN DE HISTORIAL DE MIGRACIONES (BASELINE) - V2 (SNAKE_CASE)
==============================================================================
*/

-- SincronizaciÃƒÂ³n para Configuracion.API
INSERT INTO public."__EFMigrationsHistory" ("migration_id", "product_version")
VALUES ('20260316050725_UpdateSunatFields', '8.0.0')
ON CONFLICT DO NOTHING;

-- SincronizaciÃƒÂ³n para Ventas.API
INSERT INTO public."__EFMigrationsHistory" ("migration_id", "product_version")
VALUES ('20260316050735_UpdateSunatFieldsVentas', '8.0.0')
ON CONFLICT DO NOTHING;

-- SincronizaciÃƒÂ³n para Compras.API (Ya dio ÃƒÂ©xito, pero aseguramos)
INSERT INTO public."__EFMigrationsHistory" ("migration_id", "product_version")
VALUES ('20260316050748_UpdateSunatFieldsCompras', '8.0.0')
ON CONFLICT DO NOTHING;

-- SincronizaciÃƒÂ³n para Catalogo.API
...
-- DescripciÃƒÂ³n: Registra el cÃƒÂ³digo '99' y la serie OC01 para Ãƒâ€œrdenes de Compra.
-- ==============================================================================
...
-- Script para agregar el estado 'Facturada' a las Ãƒâ€œrdenes de Compra
-- Se usa el ID 100 para evitar colisiones con otros mÃƒÂ³dulos (ej. Contabilidad que usa 43)

-- Primero eliminamos cualquier registro previo con el mismo cÃƒÂ³digo en la tabla 11
...
-- Aseguramos que el ID 43 no estÃƒÂ© asignado por error a la tabla 11
DELETE FROM configuracion.tablas_generales_detalle 
...
DescripciÃƒÂ³n: Agrega campos obligatorios UBL 2.1 a Clientes y Proveedores.
Fecha: 2026-03-27
==============================================================================
*/
...
-- 3. REGISTRO DE MIGRACION PARA EF CORE (Evita errores de desincronizaciÃƒÂ³n)
-- SincronizaciÃƒÂ³n para Compras.API
...
-- SincronizaciÃƒÂ³n para Clientes.API
...
-- *****************************************************************************
-- SCRIPT MAESTRO DE ESTABILIZACIÃƒâ€œN GLOBAL - SISTEMA COMERCIAL
-- *****************************************************************************
-- DescripciÃƒÂ³n: Este script normaliza la auditorÃƒÂ­a y estabiliza el esquema 
...
-- 1. NORMALIZACIÃƒâ€œN DE COLUMNAS DE AUDITORÃƒÂA (IDEMPOTENTE)
DO $$
DECLARE
    r RECORD;
...
BEGIN
    RAISE NOTICE 'Iniciando estabilizaciÃƒÂ³n global de auditorÃƒÂ­a...';
    ...
    LOOP
        -- AÃƒÂ±adir fecha_modificacion si no existe
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            RAISE NOTICE 'AÃƒÂ±adida fecha_modificacion a %.%', r.table_schema, r.table_name;
        END IF;

        -- AÃƒÂ±adir usuario_modificacion si no existe
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
            ...
            RAISE NOTICE 'AÃƒÂ±adida usuario_modificacion a %.%', r.table_schema, r.table_name;
        END IF;
    END LOOP;
    
    RAISE NOTICE 'EstabilizaciÃƒÂ³n global completada exitosamente.';
END $$;
...
-- *****************************************************************************
-- SCRIPT DE NORMALIZACIÃƒâ€œN GLOBAL IN-PLACE - SISTEMA COMERCIAL
-- *****************************************************************************
-- DescripciÃƒÂ³n: Este script sincroniza la base de datos PostgreSQL con el modelo 
--              de negocio (C#), normalizando tipos de fecha a UTC y tamaÃƒÂ±os 
--              de campos de auditorÃƒÂ­a sin pÃƒÂ©rdida de datos.
-- Autor: Antigravity AI (Google DeepMind)
-- Fecha: 2026-03-29
-- *****************************************************************************
...
DECLARE
    ...
    target_schemas text[] := ARRAY['ventas', 'catalogo', 'public', 'inventario', 'contabilidad', 'identidad', 'clientes', 'compras'];
BEGIN
    RAISE NOTICE 'Iniciando normalizaciÃƒÂ³n de tipos de datos y auditorÃƒÂ­a...';

    -- 1. NORMALIZACIÃƒâ€œN DE TIPOS DE FECHA Y LONGITUD DE USUARIOS
    FOR r IN 
    ...
    -- 2. ELIMINACIÃƒâ€œN DE TABLAS DE MIGRACIÃƒâ€œN DUPLICADAS
    -- Se mantiene __EFMigrationsHistory (estÃƒÂ¡ndar de .NET) y se elimina __ef_migrations_history
    ...
    RAISE NOTICE 'NormalizaciÃƒÂ³n global in-place completada exitosamente.';
END $$;

-- 3. REPORTE DE VERIFICACIÃƒâ€œN
SELECT table_schema, table_name, column_name, data_type, character_maximum_length
FROM information_schema.columns 
WHERE table_schema IN ('ventas', 'catalogo', 'public', 'inventario', 'contabilidad', 'identidad', 'clientes', 'compras')
  AND column_name IN ('fecha_creacion', 'fecha_modificacion', 'usuario_creacion', 'usuario_modificacion')
 ...
 * ImplementaciÃƒÂ³n de Notas de CrÃƒÂ©dito y DÃƒÂ©bito para el esquema de Ventas.
...
-- NormalizaciÃƒÂ³n de longitudes de usuario y campos SUNAT en ventas
...
-- AmpliaciÃƒÂ³n de auditorÃƒÂ­a en tablas relacionadas
...
-- ÃƒÂndices para optimizaciÃƒÂ³n
 ...
 * ImplementaciÃƒÂ³n de Notas de CrÃƒÂ©dito y DÃƒÂ©bito para el esquema de Compras.
...
-- AplicaciÃƒÂ³n de longitudes extendidas para auditorÃƒÂ­a en compras
...
-- Tabla de Notas de CrÃƒÂ©dito (Proveedores)
...
-- Tabla de Notas de DÃƒÂ©bito (Proveedores)
...
-- =========================================================================
-- SCRIPT DE ACTUALIZACIÃƒâ€œN: SUNAT UBL 2.1 (MÃƒâ€œDULO DE VENTAS)
-- Fecha: 2026-03-30
-- DescripciÃƒÂ³n: CreaciÃƒÂ³n de esquemas, tablas de soporte y adiciÃƒÂ³n de campos
--              para cumplir con los requerimientos de facturaciÃƒÂ³n electrÃƒÂ³nica.
-- =========================================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'sunat' AND table_name = 'cat_estado_cpe') THEN
        ...
        ('PENDIENTE', 'Pendiente de envÃƒÂ­o a SUNAT', false, true),
        ...
        ('ANULADO', 'ComunicaciÃƒÂ³n de baja aceptada', true, false),
        ('ERROR', 'Error interno o de comunicaciÃƒÂ³n', false, true);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM catalogo.unidades_medida WHERE codigo_sunat = 'MTQ') THEN
        INSERT INTO catalogo.unidades_medida (nombre_unidad, simbolo, codigo_sunat, usuario_creacion) VALUES ('Metro CÃƒÂºbico', 'm3', 'MTQ', 'sistema');
    END IF;
    ...
    -- EstandarizaciÃƒÂ³n de hash_cdr (borrar el actual y recrear si alguien creo codigo_hash_cdr)
...
-- PropÃƒÂ³sito: Generar el reporte con saldos acumulados en tiempo real.
-- =============================================
...
SELECT 
    ...
    -- SALDOS (Basados en el cÃƒÂ¡lculo al momento del registro)
...
ALTER TABLE compras.proveedores ADD ubigeo character varying(6);
                ...
                -- Ãndice GIN para bÃºsqueda rÃ¡pida por razÃ³n social (requiere pg_trgm o to_tsvector)
...
-- ImplementaciÃƒÂ³n de Anulaciones y Notas SUNAT (Esquema Ventas)
-- [2026-03-29] TraducciÃƒÂ³n a PostgreSQL

-- 1. Modificar tabla ventas para campos de anulaciÃƒÂ³n
...
-- 3. Tabla Detalle de Nota de CrÃƒÂ©dito
CREATE TABLE IF NOT EXISTS ventas.nota_credito_detalle (
...
-- 5. Tabla Detalle de Nota de DÃƒÂ©bito
CREATE TABLE IF NOT EXISTS ventas.nota_debito_detalle (
    id_detalle              BIGSERIAL PRIMARY KEY,
    id_nota_debito          BIGINT            NOT NULL,
    id_venta_detalle        BIGINT            NULL,
...
-- ImplementaciÃƒÂ³n de Anulaciones y Notas SUNAT (Esquema Compras)
-- [2026-03-29] TraducciÃƒÂ³n a PostgreSQL

-- 1. Modificar tabla compras para campos de anulaciÃƒÂ³n
ALTER TABLE compras.compras 
ADD COLUMN IF NOT EXISTS fecha_anulacion TIMESTAMP NULL,
ADD COLUMN IF NOT EXISTS motivo_anulacion TEXT NULL,
ADD COLUMN IF NOT EXISTS estado_sunat VARCHAR(20) DEFAULT 'EMITIDO';

-- 2. Tabla Nota de CrÃƒÂ©dito (Compra)
CREATE TABLE IF NOT EXISTS compras.nota_credito (
    id_nota                 BIGSERIAL PRIMARY KEY,
...
-- 3. Tabla Detalle de Nota de CrÃƒÂ©dito (Compra)
CREATE TABLE IF NOT EXISTS compras.nota_credito_detalle (
    id_detalle              BIGSERIAL PRIMARY KEY,
    id_nota_credito         BIGINT            NOT NULL,
...
-- 4. Tabla Nota de DÃƒÂ©bito (Compra)
CREATE TABLE IF NOT EXISTS compras.nota_debito (
    id_nota                 BIGSERIAL PRIMARY KEY,
    serie                   VARCHAR(10)       NOT NULL,
    numero                  VARCHAR(20)       NOT NULL,
...
-- 5. Tabla Detalle de Nota de DÃƒÂ©bito (Compra)
CREATE TABLE IF NOT EXISTS compras.nota_debito_detalle (
    id_detalle              BIGSERIAL PRIMARY KEY,
    id_nota_debito          BIGINT            NOT NULL,
    id_compra_detalle       BIGINT            NULL,
